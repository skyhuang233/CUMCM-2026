...
\textbf{针对问题二}，以分解式负载预测（日电量水平 $\times$ 日内形状）与 30 条完整路径配对残差场景构造样本平均近似，在 0:00 冻结计划购电量；执行层将 $F_t$ 推广为场景平均未来费用函数 $\bar H_t(e)$，导出动态保留水平 $R_t$——缺口时储能只放电到 $R_t$ 为止，为未来更贵的缺口留存库存。跨日前瞻配置（48 小时时域 + 次日价值函数 $V(e)$）全年实际费用 \textbf{1352.96 万元}，位于完美信息下界（1222.82 万元）之上 10.6\%，较同信息无储能方案节省 28.0\%。
...
灵敏度分析表明：场景数 $M\ge20$ 后费用进入平台期，跨日前瞻组件在四分支一致净省 2.4--4.1 万元，分解式负载预测是贡献最大的单一组件（14.5--21.2 万元）；全部参数仅用一月数据选定，杜绝信息泄漏。综上，四问是同一价值函数 $F_t\to\bar H_t\to\bar H_{t\mid\tau}\to\bar H_{t\mid\tau}(\cdot;p)$ 在信息逐级扩展下的四个实例：储能调度的本质不是追求一条标称最优轨迹，而是维护一个随信息状态更新的库存价值函数。
