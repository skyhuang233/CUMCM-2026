\paragraph{互斥消去引理}
在 $p_t\ge0$、富余供能允许不利用且不允许紧急购电的条件下，线性规划存在一个最优解满足 $C_tD_t=0$。若某段 $C_t,D_t>0$，令 $\varepsilon=\min\{C_t,D_t/\eta^2\}$，并取 $\widetilde C_t=C_t-\varepsilon$、$\widetilde D_t=D_t-\eta^2\varepsilon$。库存变化保持不变，同时释放交流侧供能 $(1-\eta^2)\varepsilon$；将其优先从 $G_t$ 中扣除，剩余部分计入 $W_t$，约束仍满足且费用不增。重复该变换即可得到互斥最优解。因此后文可用单一库存增量 $x_t=S_t-S_{t-1}$ 表示动作，而无需二元变量。
...
\begin{proof}
对 $t$ 逆向归纳。$F_{145}$ 为 $\{6000\}$ 的扩展实值指示函数，凸。设 $F_{t+1}$ 凸分段线性。记单段动作费用 $a_t(x)=p_t[L_t-V_t+\psi(x)]^+$。
