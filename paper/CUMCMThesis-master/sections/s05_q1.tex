...
图~\ref{fig:fig_f02_value_function} 上下对照 0:00、6:00、12:00、18:00 的价值函数与边际价值。相同时刻采用一致的线型与颜色，便于沿库存横轴核对凸折线及边际非增结构。

\begin{figure}[!htbp]\centering
\safeincludegraphics[width=0.96\textwidth]{fig_f02_value_function}
\caption{问题一多时刻库存价值与边际价值}\label{fig:fig_f02_value_function}
\end{figure}
...
最优计划全天购电 59482.70\,kWh、购电费 \textbf{35126.95 元}。作为对照，若不使用储能、逐段按需购电，全天费用为 48052.05 元：储能的峰谷套利把日购电费降低 12925.10 元（\textbf{26.90\%}）。指定时段结果见表~\ref{tab:q1purchase}，完整策略已存入 result1.xlsx。

\input{generated/q1_tables.tex}

\begin{figure}[!htbp]\centering
\safeincludegraphics[width=0.96\textwidth]{fig_f03_q1_day}
\caption{问题一购电功率与分时电价}\label{fig:fig_f03_q1_day}
\end{figure}


\begin{figure}[!htbp]\centering
\safeincludegraphics[width=0.96\textwidth]{fig_f14_q1_storage}
\caption{问题一充放电功率与库存轨迹}\label{fig:fig_f14_q1_storage}
\end{figure}

图~\ref{fig:fig_f03_q1_day} 与图~\ref{fig:fig_f14_q1_storage} 显示最优策略的经济机理：购电集中在电价谷段并同步充电，部分高价时段依靠光伏与放电覆盖负载、购电降至零；储电量在两个峰前充至接近上限、峰内放至接近下限，一天完成约两个完整充放循环——在效率损耗 $\eta^2=0.81$ 下，只有峰谷价差超过约 $1/0.81-1\approx23\%$ 的时段对才值得套利，这解释了为何部分中等价差时段储能保持静默。周期约束的对偶价格给出 24:00 库存的边际影子价，与次日 0:00 的 $F$ 曲线斜率一致，说明周期解自洽。
