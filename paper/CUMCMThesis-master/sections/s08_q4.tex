...
图~\ref{fig:fig_f10_price_forecast} 使用两支各自的正式预测器，比较 3 月 20 日与 12 月 21 日的 0:00 预测。4-2 两日 MAE 分别为 0.033、0.041 元/kWh，4-3 为 0.037、0.131 元/kWh。4-2 的历史回归特征按当时可得的净负荷预测重建；两日对照用于展示误差的日期与分支差异，不据此推断全年误差分布。
...
\textbf{其二，4-2 与 4-3 的全年账单差为 53.69 万元}，与问题 2、3 的 54.01 万元差距接近。图~\ref{fig:fig_f11_q4_monthly} 上图分别比较 4-2 与问 2、4-3 与问 3；下图直接给出 4-2 减 4-3 的月度账单差。两个比较使用相同的 48h 配置，但价格环境、预测规则、光伏信息与交易权限并非都相同，因此不能将账单差单独归因于某项权限或预测方法。

\begin{figure}[!htbp]\centering
\safeincludegraphics[width=0.96\textwidth]{fig_f11_q4_monthly}
\caption{问题四月度费用对照（差值方向见图例，48h 主方法）}\label{fig:fig_f11_q4_monthly}
\end{figure}
...
\caption{问题四两分支指定日库存轨迹（48h 主方法）}\label{fig:fig_f17_q4_soc}
\end{figure}
